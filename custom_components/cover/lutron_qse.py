...
from custom_components.lutron_qse import (
    CONF_IGNORE, LUTRON_QSE_INSTANCE, LUTRON_QSE_IGNORE, LutronQSEDevice)
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for roller in qse.rollers():
        if roller.serial_number in ignore:
            _LOGGER.info('Ignoring Lutron QSE Device: ' + roller.serial_number)
            continue
...
class LutronQSECover(LutronQSEDevice, CoverDevice):
    ...
    def _update(self):
        """Called when platform is notififed of state changes."""
